...
#[AsCommand(
    name: "app:simulate:club-import",
    description: "Simule l'import club + membres depuis un CSV et un logo," .
    " génère les PDF, envoie les emails, sans toucher à la base."
)]
class GenerateClubAndMemberLicencesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->writeln($clubNum
            ...
            : "Club n°: <comment>(non fourni)</comment> — un numéro sera simulé si le club est nouveau");
        ...
        if ($club) {
            ...
            $io->writeln("Saison courante simulée sur le club: <comment>$currentSeason</comment>");
        } else {
            // Demander les infos pour simuler la création
            ...
            // On SIMULE un nouvel objet Club, sans persist/flush
            $club = new Club();
            if ($clubNum === null) {
                ...
                $io->writeln("Numéro de club simulé: <info>$clubNumber</info>");
            } else {
                $club->setClubNumber($clubNum);
            }
            $club->setSportSeason($currentSeason);

            $io->success("Club INEXISTANT : simulation de création OK (AUCUNE écriture en base).");
            $io->listing([
                "Nom: {$club->getName()}",
        ...
        // --- 3) Générer la licence club (PDF) + envoyer email à marc.lassort@gmail.com
        try {
            $clubData = [
                "logo"      => $logoPath,
            ...
            $this->sendLicenseEmail($clubPdf, "club");
            $io->success("Licence CLUB générée + email envoyé à marc.lassort@gmail.com");
        } catch (TransportExceptionInterface $e) {
            $io->error("Erreur d’envoi email (club): " . $e->getMessage());
        }

        // --- 4) Lire le CSV/XLSX, simuler la création des membres, logger + email chacun
        ...
        $io->section("Traitement des membres (simulation, aucun flush)");
        ...
        foreach ($rows as $idx => $data) {
            ...
            $memberCounter++;
            $licenceNumber = $this->simulateLicenceNumber($memberCounter);

            // Objet Member uniquement en mémoire
            $member = new Member();
            ...
            // PDF + email pour chaque membre -> à Marc pour test
            try {
                $memberData = [
                ...
                $this->sendLicenseEmail($pdf, "club-membre");
                $io->writeln("→ Email licence membre envoyé à marc.lassort@gmail.com");
            } catch (TransportExceptionInterface $e) {
                $io->error("Erreur d’envoi email (membre): " . $e->getMessage());
            }
        }

        $io->success("Simulation terminée. Aucune écriture en base n’a été effectuée.");
    ...
    /** Génère un numéro de licence factice et lisible. */
    private function simulateLicenceNumber(int $counter): string
    {
     ...
     * From configuré en dur pour tes tests.
     * @throws TransportExceptionInterface
     */
    private function sendLicenseEmail(string $pdfPath, string $type): void
    {
